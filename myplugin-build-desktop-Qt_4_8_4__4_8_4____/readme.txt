...
>0. 本期推荐
保存和载入，设计中断也没关系
本来不准备自动保存的，但是出现过几次不重现的崩溃..所以加入自动保存功能，如果崩溃了请重新启动程序并提示载入
选择与循环结构归并为函数实现
文本筛选和标签筛选
pattern生成小工具（swig为何没有judgestructpattern..）
界面部分汉化，readme未汉化
>1. 快速上手
>1.1 简介
本程序希望引导用户通过人机交互生成太阳神三国杀LUA扩展文件。目前API支持ALPHA版。
>1.2 主要流程
>1.2.0 主界面
顶部有扩展包名称和翻译的输入框，回车确认
上半部分展示武将和技能的相关信息（名称翻译类型描述等），可即时修改
按钮new和delete可对武将和技能列表修改，请及时命名
按钮add添加技能行为，需选择技能
按钮undo|redo仅针对技能行为的增删
按钮import可导入设计文件
按钮export可导出LUA文件和设计文件，请先对扩展包命名
下半部分是生成的LUA文件预览，翻译表部分我加了几个自己写的函数
可选择当前技能部分预览
>1.2.1 选择设计文件
设计文件包含扩展包武将技能及翻译
设计文件格式参考mydesign.txt
编码为ANSI
不选择设计文件时，将从零开始，请注意扩展包命名
>1.2.2 新建武将与触发技
分别于武将和技能tab页使用按钮new，双击单元格可修改
>1.2.3 删除武将和触发技
按钮del，删除选中单元格对应的武将或技能
>1.2.4 导入已有设计文件
按钮import
>1.2.5 导出
按钮export，设计文件同时还包含已完成的技能行为，导入时会自动添加
>1.2.6 补充说明
目前不支持删除武将和技能的撤销..
输入框记得回车
先输名字，名字很重要，别重名
...
>1.3.3 技能信息
以>>>起始，示例 >>>luaguicai|鬼才|NotFrequent|luasmy|:你可以用手牌改判定|$天命哈哈哈|$天命哈哈哈哈哈 ，表示技能名luaguicai，翻译鬼才，非常时技能，拥有者luasmy，以及描述和台词
>1.3.4 技能行为信息
以>>>>起始，由程序生成记录技能行为，用户可以无视
>1.3.5 补充说明
之所以想加入设计文件这货，是因为我自己DIY技能时经常改来改去，文案很烦
而设计文件一定程度上省事了，发布也方便，选择设计文件前几部分即可
>1.4 技能设计流程
>1.4.0 相关概念
目前支持触发技
简单理解，需要出牌阶段用手点技能的为视为技（【仁德】），人机交互为触发技（【突袭】）
...
本程序通过Function方式实现三种方式添加行为，即程序结构的顺序选择循环三类
Function，表示相关对象Object在对应时机Event和代码段Block执行行为Function并返回结果，核心为Function（比如选择一名角色room:askForPlayerChosen|判断非延时锦囊isNDTrick）
...
>1.4.1 主界面
按钮add，弹出窗口，在多列视图中依次选择完成，点击add添加完成
input others仅支持所需数字常数和字符串常量，双击input others并输入数字即可添加所需常数
filter输入框可对末列选项及其标注进行筛选（支持正则），支持中文
filter选择框可以通过标签进行过滤
Remark展示框依次展示所选对象的标注
Remark输入框则可对本次操作产生的新对象或新代码段进行标注，方便查询
仅remark和filter支持中文
pattern generator，用于生成pattern字符串的小工具
>1.4.2 【集智】示例
技能描述：你使用非延时锦囊牌时，可立即摸一张牌
所需Function：是否技能拥有者|是否非延时锦囊isNDTrick|询问角色是否发动|摸牌drawCards
所需Event：使用牌时CardUsed
...
>1.4.3 PATTERN
模式，或者说是一种匹配标准，可以衡量所有对象符合或不符合，比如闪电的黑桃2~9就是一种pattern，再比如【享乐】的基本牌也是一种pattern
主要应用情境，判定，或是奇怪的选择条件（请选择一张黑桃2~9）
使用方法，将符合的单元格依次选中，选择generate可生成，再选择copy可复制
注意，目前API下，判定pattern特殊，且一般只考虑花色和点数，使用时请点选“Judge”
黑桃1~7或红桃8~13，请点选“or”
>1.4.4 补充说明
目前支持Object类型：myplayer|mycard|mynum|mystr|mybool|myplayerlist|mycardlist|mynumlist
